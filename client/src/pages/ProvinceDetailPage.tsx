...
export default function ProvinceDetailPage() {
  ...
  return (
    <div>
      <section className="relative flex min-h-[52vh] items-end overflow-hidden bg-pine text-white">
        <img
        ...
        <div className="py-16">
          <div className="flex flex-wrap gap-2">
            {province.highlights.map((highlight) => (
              <span
                key={highlight}
                className="inline-flex items-center gap-1.5 rounded-full border border-sage/40 bg-sage/10 px-3.5 py-1.5 text-sm font-medium text-ink"
              >
      ...
      <section className="bg-mint">
        <div className="mx-auto max-w-6xl px-4 py-20 sm:px-6">
          ...
          {visible.length === 0 ? (
            <p className="mt-12 text-center text-mist">{t('provinceDetail.empty')}</p>
          ) : (
